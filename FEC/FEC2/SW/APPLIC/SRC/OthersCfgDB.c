...
extern SUBSYS_ID GlobalSubSysId;
...
static int Convert12bitDataInto4_20ma(int DataDig)
{
    ...
    M_LOGGER_LOGF(M_LOGGER_LEVEL_DEBUG,"Convert12bitDataInto4_20ma Dig data=%d, Analog data=%d",DataDig,DataAnalog);
...
static void MB_GetDataResponseHandler(uint8_t func_status, uint16_t *resp_buf, int32_t resp_sz,uint32_t RequestDeviceSequence )
{
    ...
    {
        M_LOGGER_LOGF(M_LOGGER_LEVEL_ERROR,"OTHERS_UPDATE_DB_RESP ERROR: func_status=%d",func_status);
        return;
    }


    M_LOGGER_LOGF(M_LOGGER_LEVEL_DEBUG,"START ->OTHERS_UPDATE_DB_RESP. total_ModbusRetries=%d",get_total_retries());
    ...
    for (int i=0;i<resp_sz;i++)
    {
        ...
        {
            M_LOGGER_LOGF(M_LOGGER_LEVEL_ERROR,"********ERROR CfgPutPara:  OTHERS_UPDATE_DB_RESP:  InstanceId=%d, DB_Id=%s, data=%d ",InstanceId,Id2Str(CfgDB_ID_AddIndex(DB_Id,DB_StartIdIndex+i), IdStr),data);

            //while (1) {};
        }
        M_LOGGER_LOGF(M_LOGGER_LEVEL_DEBUG,"OTHERS_UPDATE_DB_RESP: InstanceId=%d, DB_Id=%s, data=%d ",InstanceId,Id2Str(CfgDB_ID_AddIndex(DB_Id,DB_StartIdIndex+i), IdStr),data);
    } //for
...
static STATUS Set_ID_OTHERS_CMND_INT_UPDATE_DB_PERIPHERALS_VALUES_handle(int InstanceNum,int Index,CfgDB_INT_DATA Data,CfgDB_SET_FLAG *SetFlag)
{
    ...
    if (IsInAtpMode())
    {
        ...
        int Index;
        //digital inputs
        for (Index=0;Index<NUM_OF_DIGITAL_INP;Index++)
            CfgGetParam(NULL, CfgDB_ID_AddIndex(ID_OTHERS_MNTR_INT_DIGITAL_INP_n,Index),&TempData,sizeof(TempData), CfgDB_GET_DIRECTLY_FROM_SOURCE);
        //analog inputs digital ma
        for (Index=0;Index<MAX_AVAILIABLE_MA_DIG;Index++)  //analog inputs digital ma
        {
            CfgGetParam(NULL, CfgDB_ID_AddIndex(ID_OTHERS_MNTR_INT_ANALOG_INP_MA_n,Index),&TempData,sizeof(TempData), CfgDB_GET_DIRECTLY_FROM_SOURCE);
            //TBD - convert to 4-20 ma and store in ID_OTHERS_MNTR_INT_ANALOG_INP_MA_n
        }

        //analog inputs digital Humidity sensor
        for (Index=0;Index<NUMBER_OF_ANALOG_INP_HM;Index++)  //analog inputs digital Humidity sensor
        {
            CfgGetParam(NULL, CfgDB_ID_AddIndex(ID_OTHERS_MNTR_INT_ANALOG_INP_HM_MA_n,Index),&TempData,sizeof(TempData), CfgDB_GET_DIRECTLY_FROM_SOURCE);
            //TBD - convert to TBD and store in ID_OTHERS_MNTR_INT_ANALOG_INP_HM_MA_n
        }
    }
    else
    {  // External zone handle
        ...
        M_LOGGER_LOGF(M_LOGGER_LEVEL_DEBUG,"START ->OTHERS_UPDATE_DB_REQ");
        ReqSequance=0;
        while (PeripheralsModBusDescriptionPtr[ReqSequance])
        {
            ...
            M_LOGGER_LOGF(M_LOGGER_LEVEL_DEBUG,"OTHERS_UPDATE_DB_REQ: ReqSequance=%d, DeviceId=%d, RegisterId=%d, NumberOfElements=%d ",ReqSequance,DeviceId,RegisterId,NumberOfElements);
            ReqSequance=ReqSequance+=1;
        }
        M_LOGGER_LOGF(M_LOGGER_LEVEL_DEBUG,"END ->OTHERS_UPDATE_DB_REQ. ReqSequance=%d",ReqSequance);
    }
    return OK;
}
...
{ 
    Set_AO_mA((u8)Index, (u16) Data * 100);
    return OK;
}  

static STATUS Set_ID_OTHERS_CMND_INT_ANALOG_OUT_V_handle(int InstanceNum,int Index,CfgDB_INT_DATA Data,CfgDB_SET_FLAG *SetFlag)
{  
    Set_AO_V((u16)Data * 100);
    return OK;
}  

static STATUS Set_ID_OTHERS_CMND_INT_SAFTY_RELAY_handle(int InstanceNum,int Index,CfgDB_INT_DATA Data,CfgDB_SET_FLAG *SetFlag)
{
    //M_LOGGER_LOGF(M_LOGGER_LEVEL_TRACE,"SAFETY_CTRL:  Data=%d ",Data);
    Safety_out_ctrl((OPTO_OUT_CTRL_VAL_ET) Data);
    Turn_led_(LABL_SAFETY_LED9, (BOOL) Data);  //also turn the safety led

    return OK;
}

static STATUS Set_ID_OTHERS_CMND_INT_LED_OUT_n_handle(int InstanceNum,int Index,CfgDB_INT_DATA Data,CfgDB_SET_FLAG *SetFlag)
{


    //M_LOGGER_LOGF (M_LOGGER_LEVEL_TRACE,"TURN_LED_#%d, %d ", Index, Data);
    Turn_led_((LED_ID_ET) Index, (BOOL) Data);
    return OK;
}

static STATUS Set_ID_OTHERS_CMND_INT_ANALOG_IN_MA_CALIB_n_handle(int InstanceNum,int Index,CfgDB_INT_DATA Data,CfgDB_SET_FLAG *SetFlag)
{
    if (IsInAtpMode())
        AnalogMaCalibrationSetData(Index,Data);
    return OK;
}

static STATUS Set_ID_OTHERS_CMND_INT_ITH_VALVES_STATE_handle(int InstanceNum,int Index,CfgDB_INT_DATA Data,CfgDB_SET_FLAG *SetFlag)
{
    dig_out_ctrl(7, Data & 0x1);  //turn on/off  valve#1
    dig_out_ctrl(8, ((int)Data>>1) & 0x1);  //turn on/off  valve#2
    return OK;
}

/************************************* MONITOR INT ******************************************************/
static STATUS Get_ID_OTHERS_MNTR_INT_DIGITAL_INP_n_Handle(int InstanceNum,int Index,int* Data)
{
    uint32_t res;
    ...
    if (dig_in_get((DIG_IN_ID_ET)Index, &DataByte)!=LL_ERROR)
    {
        *Data=DataByte;
        return OK;
    }
...
static STATUS Get_ID_OTHERS_MNTR_INT_ANALOG_INP_MA_n_Handle(int InstanceNum,int Index,int* Data)
{
    A2D_RES_ET  res;
    ...
    // *Data+=AnalogMaCalibrationData(Index);  //add the ma calibration value

    res = a2d_read_dig( (A2D_DATA_ET) (Index + AN_IN_4_20_mA_INDX_OFFSET), &TmpData);

    // M_LOGGER_LOGF (M_LOGGER_LEVEL_TRACE,"ANL_MA_IN_#%d without calib, %d ", Index, TmpData);
    *Data = (uint32_t)(10*TmpData*3.3*1000/4096/165);

    //M_LOGGER_LOGF (M_LOGGER_LEVEL_TRACE,"ANL_MA_IN_#%d, after calib %d ", Index, *Data);

    return OK;

}

static STATUS Get_ID_OTHERS_MNTR_INT_ANALOG_INP_HM_MA_n_Handle(int InstanceNum,int Index,int* Data)
{
    A2D_RES_ET  res;
    uint16_t TmpData;
    ...
    res = a2d_read_dig( (A2D_DATA_ET) (Index + HM_SENS_INDX_OFFSET), &TmpData);

    //M_LOGGER_LOGF (M_LOGGER_LEVEL_TRACE,"ANL_HM_DIG_#%d without calib, %d ", Index, TmpData);
...
{ /* default values */
        ...
        {"ID_OTHERS_CMND_INT_ANALOG_OUT_MA_n",              INFORM_NOTHING, 40,    200,  UNSIGNED_VALUE, NUM_OF_ANALOG_OUT_MA, 40,    NULL, Set_ID_OTHERS_CMND_INT_ANALOG_OUT_MA_n_handle},
        /*ID_OTHERS_CMND_INT_ANALOG_OUT_V*/
        {"ID_OTHERS_CMND_INT_ANALOG_OUT_V",                 INFORM_NOTHING, 0,     100,  UNSIGNED_VALUE, 1,                  0,     NULL, Set_ID_OTHERS_CMND_INT_ANALOG_OUT_V_handle},
        /*ID_OTHERS_CMND_INT_SAFTY_RELAY*/
        {"ID_OTHERS_CMND_INT_SAFTY_RELAY",                  INFORM_NOTHING, FALSE, TRUE, UNSIGNED_VALUE, 1,                  FALSE, NULL, Set_ID_OTHERS_CMND_INT_SAFTY_RELAY_handle},
        /*ID_OTHERS_CMND_INT_LED_OUT_n*/
        {"ID_OTHERS_CMND_INT_LED_OUT_n",                    INFORM_NOTHING, FALSE, TRUE, UNSIGNED_VALUE, NUMBER_OF_LEDS,       FALSE, NULL, Set_ID_OTHERS_CMND_INT_LED_OUT_n_handle},
        /*ID_OTHERS_CMND_INT_ANALOG_IN_MA_CALIB_n*/
        {"ID_OTHERS_CMND_INT_ANALOG_IN_MA_CALIB_n",         INFORM_NOTHING, -100,  100,  SIGNED_VALUE,   NUM_OF_ANALOG_OUT_MA, 0,     NULL, Set_ID_OTHERS_CMND_INT_ANALOG_IN_MA_CALIB_n_handle},
        /*ID_OTHERS_CMND_INT_ITH_VALVES_STATE*/
        {"ID_OTHERS_CMND_INT_ITH_VALVES_STATE",         INFORM_NOTHING, 0,  3,  UNSIGNED_VALUE,   1, 0,     NULL, Set_ID_OTHERS_CMND_INT_ITH_VALVES_STATE_handle},
...
{ /* default values */
        ...
        { "ID_OTHERS_MNTR_INT_DIGITAL_INP_n",      INFORM_NOTHING, 0,                     1,                     UNSIGNED_VALUE, NUM_OF_DIGITAL_INP,      0, Get_ID_OTHERS_MNTR_INT_DIGITAL_INP_n_Handle,      NULL},
        /* ID_OTHERS_MNTR_INT_ANALOG_INP_MA_n */
        { "ID_OTHERS_MNTR_INT_ANALOG_INP_MA_n",    INFORM_NOTHING, MIN_AVAILIABLE_MA,     MAX_AVAILIABLE_MA,     UNSIGNED_VALUE, NUMBER_OF_ANALOG_INP_MA, 0, Get_ID_OTHERS_MNTR_INT_ANALOG_INP_MA_n_Handle,    NULL},
        ...
        {"ID_OTHERS_MNTR_INT_ANALOG_INP_HM_MA_n",  INFORM_NOTHING, MIN_AVAILIABLE_MA,     MAX_AVAILIABLE_MA,     UNSIGNED_VALUE, NUMBER_OF_ANALOG_INP_HM, 0, Get_ID_OTHERS_MNTR_INT_ANALOG_INP_HM_MA_n_Handle, NULL},
...
STATUS OthersCfgBuildDB(BOOL SkipDefaultFile)
{
    ...
    if (!SkipDefaultFile)
    {
        ...
        {
            RemoveRamDataBase();
            M_LOGGER_LOGF (M_LOGGER_LEVEL_ERROR,"OthersCfgInit- Error form CreateRamDataBase. ");
            return ERROR;
        }
    }
    ...
    {
        M_LOGGER_LOGF (M_LOGGER_LEVEL_ERROR,"OthersCfgInit- Error form CfgDB_FindFilesPathAndNames for integer type id=0x%x. ", id.DataAssign);
        return RetValue;
    }
    ...
    {
        M_LOGGER_LOGF (M_LOGGER_LEVEL_ERROR,"OthersCfgInit- Error form FileHandCreateAndInitCfgIntFileDB. RetValue=%d", RetValue);
    }
    ...
    if ((RetValue=CfgDB_FindFilesPathAndNames(id,DefaultFileName,ModifiedFileName,0))!=OK) /* find the exact name*/
    {
        M_LOGGER_LOGF (M_LOGGER_LEVEL_ERROR,"OthersCfgInit- Error form CfgDB_FindFilesPathAndNames for string type id=0x%x. ", id);
    ...
    {
        M_LOGGER_LOGF (M_LOGGER_LEVEL_ERROR,"OthersCfgInit- Error form FileHandCreateAndInitCfgStrFileDB. RetValue=%d", RetValue);
    }
    CfgDB_GiveCfgSem(CARRIER_OTHERS);
#endif
    return OK;
...
STATUS OthersCfgUpdateDbFromSource( BOOL ConfigMismatchCheck,BOOL LoadConfigFromUnit)
{
            ...
            &OthersDbIntDescriptor[0],ID_OTHERS_CFG_INT_FIRST, ID_OTHERS_CFG_INT_LAST,ConfigMismatchCheck,LoadConfigFromUnit))!=OK)
    {
        if (RetValue!=CFG_DB_ERROR_ID_CONFIG_MISMATCH)
            M_LOGGER_LOGF (M_LOGGER_LEVEL_ERROR,"ERROR in OthersCfgUpdateDbFromSource: CfgDB_UpdateIntDbFromSource RetValue=0x%x",RetValue);
    }
...
STATUS OthersCfgInit()
{
    ...
    {
        M_LOGGER_LOGF (M_LOGGER_LEVEL_ERROR,"OthersCfgInit- Error form AlarmOTHERS_Init. ");
        return RetValue;
    }
#endif

    if ((RetValue=OthersCfgBuildDB(FALSE))!=OK)
        M_LOGGER_LOGF (M_LOGGER_LEVEL_ERROR,"ERROR in OthersCfgBuildDB: RetValue=0x%x",RetValue);
    ...
    {
        M_LOGGER_LOGF (M_LOGGER_LEVEL_ERROR,"OthersCfgInit- Error form OthersCfg_UpdatePowerUpMntrDB. RetValue=%d ", RetValue);
    }
    ...
    {
        M_LOGGER_LOGF (M_LOGGER_LEVEL_ERROR,"OthersCfgInit- Error form CfgDB_CallToAllStrSetHandlesAndInformClients. RetValue=%d ", RetValue);
    }

    /* call to all relevant set handle of each entry in the Cfg int DB (only those which has one)
	for completed updating */
    ...
    {
        M_LOGGER_LOGF (M_LOGGER_LEVEL_ERROR,"OthersCfgInit- Error form CfgDB_CallToAllIntSetHandlesAndInformClients. RetValue=%d", RetValue);
    }

    /* call to all relevant set handle of each entry in the Cmnd int DB (only those which has one)
	for completed updating - skip the first one - UPDATE_DB_PERIPHERALS_VALUES*/
    if ((RetValue=CfgDB_CallToAllIntSetHandlesAndInformClients(0,&OthersDbCmndIntPtr[1],
            &OthersDbCmndIntDescriptor[1],
            (ID_OTHERS_CMND_INT_FIRST)+1, ID_OTHERS_CMND_INT_LAST))!=OK)
    {
        M_LOGGER_LOGF (M_LOGGER_LEVEL_ERROR,"OthersCfgInit- Error form CfgDB_CallToAllIntSetHandlesAndInformClients. RetValue=%d", RetValue);
    }
...
static STATUS OthersCfg_UpdatePowerUpMntrDB()
{
    ...
    if ((RetValue=CfgDB_UpdateIntDbFromSource(0,&OthersDbMntrIntPtr[0],
            &OthersDbMntrIntDescriptor[0],ID_OTHERS_MNTR_INT_FIRST,
            ID_OTHERS_MNTR_INT_LAST,FALSE,FALSE))!=OK)
    {
        M_LOGGER_LOGF (M_LOGGER_LEVEL_ERROR,"ERROR in OthersCfg_UpdatePowerUpMntrDB: CfgDB_UpdateIntDbFromSource ");
        return RetValue;
    }
...
STATUS OthersCfgUpdateEepromStruct()
{
    ...
    {
        M_LOGGER_LOGF (M_LOGGER_LEVEL_ERROR,"ERROR in OthersCfgUpdateEepromStruct: in EEPROM_GetHwConfigurationValues");
        return RetValue;
    }
    if ((RetValue=EEPROM_GetBoardIdValues(NULL,BOARD_OTHERS,&ProductionBoardIdStruct))!=OK)
    {
        M_LOGGER_LOGF (M_LOGGER_LEVEL_ERROR,"ERROR in OthersCfgUpdateEepromStruct: in EEPROM_GetBoardIdValues");
        return RetValue;
    }
...
#endif  
    if (RetValue==OK)
        M_LOGGER_LOGF (M_LOGGER_LEVEL_TRACE,"OthersCfgUpdateEepromStruct was update");
    return RetValue;
