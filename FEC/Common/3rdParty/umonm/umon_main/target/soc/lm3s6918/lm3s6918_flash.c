...
#define SSIZE			1024
#define SECTORTOT		256
#define DUMMY_FLASHID	9999

int
lm3s6918_erase(struct flashinfo *fdev, int snum)
{
	unsigned char *base;

	sectortoaddr(snum,0,&base);
	if (FlashErase((unsigned long)base) == 0)
		return(0);
	return(-1);
}

int
lm3s6918_write(struct flashinfo *fdev,unsigned char *dest,unsigned char *src,long bytecnt)
{
	int i, leftover;
	unsigned char *ldest, lsrc[4];
	...
	if (FlashProgram((unsigned long *)src,(unsigned long)dest,(unsigned long)bytecnt) != 0)
		return(-1);
	...
	if (leftover) {
		...
		if (FlashProgram((unsigned long *)lsrc,(unsigned long)(dest+bytecnt),4) == 0)
			return(0);
	}
...
lm3s6918_write(struct flashinfo *fdev,unsigned char *dest,unsigned char *src,long bytecnt)
{
	int rc = _lm3s6918_write(fdev,dest,src,bytecnt);

	printf("fwrite(0x%x,0x%x,%d) = %d\n",dest,src,bytecnt,rc);
	return(rc);
}
#endif

int
lm3s6918_ewrite(struct flashinfo *fdev,unsigned char *dest,unsigned char *src,long bytecnt)
{
...
int
lm3s6918_type(struct flashinfo *fdev)
{
...
int
lm3s6918_lock(struct flashinfo *fdev,int snum,int operation)
{
...
struct flashdesc FlashNamId[] = {
	{ DUMMY_FLASHID,	"LM3S6918-Flash"}
};

struct sectorinfo sinfoLM3S6918[SECTORTOT];
...
FlashInit(void)
{
	...
	fbnk->fltype = lm3s6918_type;
	fbnk->flerase = lm3s6918_erase;
	fbnk->flwrite = lm3s6918_write;
	fbnk->flewrite = lm3s6918_ewrite;
	fbnk->fllock = lm3s6918_lock;
	fbnk->fllock = FlashLockNotSupported;

	fbnk->sectors = sinfoLM3S6918;
