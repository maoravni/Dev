...
$(SOCSOBJ):
	$(CC) $(ASMFLAGS) -o ${@F} $(SOCDIR)/$(@:%.o=%.S)

$(SOCCOBJ):
	$(CC) $(CFLAGS) -o $@ $(SOCDIR)/$(@:%.o=%.c)
...
$(DEPEND):	
	...
	then \
		...
		for obj in $(SOCSSRC); do \
			$(CC) $(COMMON_CFLAGS) $(CUSTOM_CFLAGS) $(COMMON_INCLUDE) \
				$(CUSTOM_INCLUDE) -MM $$obj >>$(DEPENDFILE) ; \
			echo $$obj dependencies... ; \
		done; \
		for obj in $(SOCCSRC); do \
			$(CC) $(COMMON_CFLAGS) $(CUSTOM_CFLAGS) $(COMMON_INCLUDE) \
				$(CUSTOM_INCLUDE) -MM $$obj >>$(DEPENDFILE) ; \
			echo $$obj dependencies... ; \
		done; \
...
#
clobber: clean clobber_local
	rm -f *.o *.a
	rm -rf $(BUILDDIR) tags cscope* *.gz depends 
...
#
cscope: cscope_local
	for file in $(LOCSSRC); do ls $$file \
		>>cscope.files; done 
	for file in $(LOCCSRC); do ls $$file \
		>>cscope.files; done 
	for file in $(SOCSSRC); do ls $$file \
		>>cscope.files; done 
	for file in $(SOCCSRC); do ls $$file \
		>>cscope.files; done 
	for file in $(CPUSSRC); do ls $(CPUDIR)/$$file \
		>>cscope.files; done 
	for file in $(CPUCSRC); do ls $(CPUDIR)/$$file \
		>>cscope.files; done 
	for file in $(COMCSRC); do ls $(COMDIR)/$$file \
		>>cscope.files; done 
	for file in $(GLIBSRC); do ls $(GLIBDIR)/$$file \
		>>cscope.files; done 
	for file in $(ZLIBSRC); do ls $(ZLIBDIR)/$$file \
		>>cscope.files; done 
