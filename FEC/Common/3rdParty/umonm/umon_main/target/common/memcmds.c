...
Pm(int argc,char *argv[])
{
	...
	ushort	val2;
	uchar	val1, c;
	int	opt, width, ascii, fifo, i, j, concatenate, pmop;

	pmop = PM_EQ_OPERATION;
	width = 1;
	ascii = fifo = 0;
	concatenate = 0;
	while((opt=getopt(argc,argv,"24afosSx")) != -1) {
		switch(opt) {
		case '2':
			width = 2;
			break;
...
Dm(int argc,char *argv[])
{
	...
	bin_display = BD_NULL;
	varname = (char *)0;
	while((opt=getopt(argc,argv,"24bf:mv:")) != -1) {
		switch(opt) {
		case '2':
			width = 2;
			break;
	...
	do {
		...
		if (width == 1) {
			...
			if (varname) {
				shell_sprintf(varname,vprfmt,*sp);
			}
			else {
				while(count>0) {
			...
			if (varname) {
				shell_sprintf(varname,vprfmt,*lp);
			}
			else {
				while(count>0) {
