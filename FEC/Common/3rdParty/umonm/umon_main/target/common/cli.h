...
#ifndef INCLUDE_RESETCMD
#define INCLUDE_RESETCMD 1
#endif
#ifndef INCLUDE_CALLCMD
...
/* externals available if YAFFS is included in the build:
 */
extern int yaffscli_copy_tfs2yaffs(const char *toYaffs,const char *fromTfs);
extern int yaffscli_copy(const char *toYaffs,const char *fromYaffs);
extern int yaffscli_ls(const char *dname,int recursive, int details);
extern int yaffscli_add(char *name, int flags, int mode, char *src, int size);
