...
#endif
#if INCLUDE_DHCPBOOT
ushort DhcpClientPort, DhcpServerPort;
#endif
ushort	TftpPort, TftpSrcPort;
...
struct pinfo icmpv6types[] = {
	{ 128, "Echo rqst" },
	{ 129, "Echo reply" },
	{ 133, "Router solicit" },
	{ 134, "Router advertise" },
	{ 135, "Neighbor solicit" },
	{ 136, "Neighbor advertise" },
	{ 137, "Redirect" },
	{ 0,0 }
};
...
char *EtherHelp[] = {
	...
	"",
	"Commands...",
	"   {on | off | mac | stat | {print I|i|O pkt len}}",
#endif
	0
};
...
#if INCLUDE_IPV6
	MacToLinkLocalIPV6Addr(BinIpv6Addr,(uint8_t *)BinEnetAddr);
#endif
...
#ifdef DEFAULT_NETMASK
	if (getenv("NETMASK") == (char *)0)
...
#if INCLUDE_IPV6
uint8_t IPv6MulticastOnly;

int 
thisPacketIsIPMulticast(struct ether_header *ehdr)
{
#if 0
	if ((ehdr->ether_dhost.ether_addr_octet[0] == 0x01) &&
		(ehdr->ether_dhost.ether_addr_octet[1] == 0x00) &&
		(ehdr->ether_dhost.ether_addr_octet[2] == 0x5e) &&
		((ehdr->ether_dhost.ether_addr_octet[3] & 0x80) == 0))
		return(1);
#endif
	if ((ehdr->ether_dhost.ether_addr_octet[0] & 0x01) == 0x01)
		return(1);
	return(0);
}

int
thisPacketIsIPv6Multicast(struct ether_header *ehdr)
{
	if ((ehdr->ether_dhost.ether_addr_octet[0] == 0x33) &&
		(ehdr->ether_dhost.ether_addr_octet[1] == 0x33))
		return(1);
	return(0);
}
#endif
...
#if INCLUDE_IPV6
	if (IPv6MulticastOnly) {
		if (thisPacketIsIPMulticast(ehdr)) {
			if (!thisPacketIsIPv6Multicast(ehdr))
				return;
		}
	}
#endif
...
#if INCLUDE_IPV6
	else if (ehdr->ether_type == ecs(ETHERTYPE_IPV6)) {
		processIPv6(ehdr,size);
		return;
	}
#endif
	else if (ehdr->ether_type != ecs(ETHERTYPE_IPV4)) {
		return;
	}
...
processMONCMD(struct ether_header *ehdr,ushort size)
{
	...
	struct	Udphdr *uhdr;
	char	*moncmd, buf[48];
	char 	*(*ip2str)(uint8_t *,char *);
	uchar	*src;

	if (size > sizeof(IPMonCmdHdrBuf))
		return;
#if INCLUDE_IPV6
	if (ehdr->ether_type == ecs(ETHERTYPE_IPV6)) {
		struct	ipv6hdr *i6hdr;

		i6hdr = (struct ipv6hdr *)(ehdr + 1);
		uhdr = (struct Udphdr *)(i6hdr + 1);
		moncmd = (char *)(uhdr + 1);
		ip2str = Ipv6ToString;
		src = (uchar *)&i6hdr->saddr;
	}
	else
#endif
	{
		...
		uhdr = (struct Udphdr *)(ihdr + 1);
		moncmd = (char *)(uhdr + 1);
		ip2str = (char *(*)(uint8_t *,char *))IpToString;
		src = (uchar *)ihdr->ip_src.s_addr;
	}
	...
	printf("MONCMD from %s: <%s>\n",ip2str((uint8_t *)src,buf),moncmd);
...
#if INCLUDE_IPV6
	if (te->ether_type == ecs(ETHERTYPE_IPV6)) {
		...
		ip6ptmp = (struct ipv6hdr *)(IPMonCmdHdr+1);
		memcpy((void *)ip6p->daddr,(void *)ip6ptmp->saddr,IPV6ADDRSZ);
		memcpy((void *)ip6p->saddr,(void *)ip6ptmp->daddr,IPV6ADDRSZ);
		ip6p->plen = ecs((sizeof(struct Udphdr) + idx));
		ip6p->hoplmt = 255;
		ip6p->nxthdr = IP_UDP;
		ip6p->vtf = ecl(0x60000000);

		tu = (struct Udphdr *) (ip6p + 1);
		ru = (struct Udphdr *) (ip6ptmp + 1);
		tu->uh_sport = ru->uh_dport;
		tu->uh_dport = ru->uh_sport;
		tu->uh_ulen = ip6p->plen;
		memcpy((char *)(tu+1),linebuf,idx);
	
		udp6Chksum(ip6p);
		sendBuffer(sizeof(struct ether_header) + sizeof(struct ipv6hdr) + sizeof(struct Udphdr) + idx);
	}
	else
#endif
	{
...
#if INCLUDE_IPV6
	case ecs(ETHERTYPE_IPV6):
		printIpv6((struct ipv6hdr *)(ehdr+1));
		break;
#endif
	case ecs(ETHERTYPE_IPV4):
...
int
showProto(int proto, char *fragmented)
{
	int i;

	for(i=0;protocols[i].pname;i++) {
		if (proto == protocols[i].pnum) {
...
printIp(struct ip *ihdr)
{
	...
	return(showProto(icpy->ip_p,fragmented));
}


int
printICMPv6(struct icmpv6hdr *icmpv6p)
{
	int i;

	printf("   ICMPv6: ");
	for(i=0;icmpv6types[i].pname;i++) {
		if (icmpv6p->type == icmpv6types[i].pnum) {
			printf("%s\n",icmpv6types[i].pname);
			return(0);
		}
	}
...
int
printIpv6(struct ipv6hdr *ipv6p)
{
	int len;
	char *fragmented = "";
	uint32_t vsn_tc_flowlbl;

	len = ecs(ipv6p->plen);
	vsn_tc_flowlbl = ecl(ipv6p->vtf);

	printf("  IPv6 pkt...\n");
	printf("   Vsn: %02x, Traffic-class: 0x%02x, flow label: 0x%06x\n",
		IPV6_VERSION(vsn_tc_flowlbl),
		IPV6_TRAFFIC_CLASS(vsn_tc_flowlbl),
		IPV6_FLOW_LABEL(vsn_tc_flowlbl));
	printf("   Nxthdr: %d, Payload len: %d, hoplimit: %d\n",
		ipv6p->nxthdr, len,ipv6p->hoplmt);
#if INCLUDE_IPV6
	showIPV6Addr("   From: ",ipv6p->saddr,"\n");
	showIPV6Addr("   To:   ",ipv6p->daddr,"\n");

	if (ipv6p->nxthdr == IP_UDP) {
		printUdp((struct Udphdr *)(ipv6p+1),fragmented);
		return(0);
	}
	else if (ipv6p->nxthdr == IP_ICMPV6) {
		printICMPv6((struct icmpv6hdr *)(ipv6p+1));
		return(0);
	}
	else if (ipv6p->nxthdr == IP_IGMP) {
		printIgmp((struct Igmphdr *)(ipv6p+1),fragmented);
		return(0);
	}
#endif

	return(showProto(ipv6p->nxthdr,fragmented));
}
...
#if INCLUDE_IPV6
/* Ipv6ToString():
 *	Incoming ascii pointer is assumed to be pointing to a buffer
 *  that can hold a complete IPV6 address (40 bytes).
 */
char *
Ipv6ToString(uint8_t *ipv6addr,char *ascii)
{
	int i;
	char *base = ascii;
	
	for(i=0;i<IPV6ADDRSZ;i++) {
		ascii += sprintf(ascii,"%02x",ipv6addr[i]);
		if ((i & 1) && (i < 15))
			strcat(ascii++,":");
	}
	return(base);
...
void
showIPV6Addr(char *prefix, uint8_t *ipv6addr, char *suffix)
{
	int i;
...
int
Ipv6ToBin(char *ascii,uint8_t *binary)
{
	uint16_t	digit;
	char		*base, *cp, tmp[5];
	int			ctot, rc, i, j;
	...
	for(i=0;i<8;i++) {
		...
		if (cp == tmp)
			break;
		*cp = 0;
		digit = (uint16_t)strtol(tmp,0,16);
		binary[j++] = (uint8_t)((digit & 0xff00) >> 8);
		binary[j++] = (uint8_t)(digit & 0xff);
		ascii++;
		if (*ascii == ':') {	// Double-colon?
			if (ctot == 7)
				goto done;
			while(ctot <= 7) {
				binary[j++] = 0;
	...
	if (rc == -1)
		printf("IPV6 addr didn't parse: %s\n",base);
...
void
udp6Chksum(struct ipv6hdr *ihdr)
{
	register int	i;
	register ushort	*datap;
	register long	sum;
	int	 	len;
	struct	Udphdr *uhdr;
	struct	Udp6Pseudohdr	pseudohdr;
	...
	/* Start with the checksum of the pseudo header:
	 */
	memcpy((char *)pseudohdr.saddr,(char *)ihdr->saddr,IPV6ADDRSZ);
	memcpy((char *)pseudohdr.daddr,(char *)ihdr->daddr,IPV6ADDRSZ);
	pseudohdr.ulplen = ihdr->plen;
	pseudohdr.nxthdr = ihdr->nxthdr;
	self_ecl(pseudohdr.nxthdr);

	/* Get checksum of pseudo header: */
	sum = 0;
	datap = (ushort *) &pseudohdr;
	for (i=0;i<(sizeof(struct Udp6Pseudohdr)/sizeof(ushort));i++) {
		sum += *datap++;
		if (sum & 0x80000000)
			sum = (sum & 0xffff) + (sum >> 16);
	}
