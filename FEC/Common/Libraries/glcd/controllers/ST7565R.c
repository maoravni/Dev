...
// taken from https://graphics.stanford.edu/~seander/bithacks.html#BitReverseTable
...
void glcd_flip()
{
    static int bufSize = (GLCD_LCD_WIDTH * GLCD_LCD_HEIGHT / 8);
    uint8_t a, b, temp;
    for (int i = 0; i < bufSize / 2; ++i)
    {
        a = glcd_buffer[i];
        b = glcd_buffer[bufSize - i];

        a = BitReverseTable256[a];
        ...
        glcd_buffer[i] = a;
        glcd_buffer[bufSize - i] = b;
    }

    if (glcd_bbox.x_max >= glcd_bbox.x_min)
    {
        uint8_t xsize, ysize, xoffset, yoffset;
        xsize = glcd_bbox.x_max - glcd_bbox.x_min;
        ysize = glcd_bbox.y_max - glcd_bbox.y_min;
        xoffset = glcd_bbox.x_min;
        yoffset = glcd_bbox.y_min;

        glcd_bbox.x_min = GLCD_LCD_WIDTH - xoffset - ysize;
        glcd_bbox.x_max = GLCD_LCD_WIDTH - xoffset;
        glcd_bbox.y_min = GLCD_LCD_HEIGHT - yoffset - ysize;
        glcd_bbox.y_max = GLCD_LCD_HEIGHT - yoffset;
    }
}
...
void glcd_write()
{
	...
	for (bank = 0; bank < GLCD_NUMBER_OF_BANKS; bank++) {
		...
		glcd_set_x_address(glcd_bbox_selected->x_min+4);
...
void glcd_ST7565R_init(void) {
	...
	glcd_command(ST7565R_REVERSE); /* ADC select in normal mode */
	glcd_command(ST7565R_DISPLAY_OFF); /* Display OFF */
	...
	glcd_command(ST7565R_DISPLAY_ON); /* Display on */
