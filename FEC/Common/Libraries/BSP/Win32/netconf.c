...
#include <stdio.h>
#include "../../STM32F4x7_ETH_LwIP/Utilities/Third_Party/lwip-1.4.1/contrib/ports/win32/pcapif.h"
...
/**
  * @brief  Change the IP-address value.
  * @param  Bytes of the IP-address value: Keep the order - new_ip_addr_0 should
  * @param                                be a Most Significant IP-address Byte!
  * @retval None
  */
void 
  change_ip_addr(
                 ...
                 )
  {
    ...
    IP4_ADDR(&ipaddr, new_ip_addr_0, new_ip_addr_1, new_ip_addr_2, new_ip_addr_3);
...
void LwIP_Init(void)
{
  ...
  netif_add(&xnetif, &ipaddr, &netmask, &gw, NULL, &pcapif_init, &tcpip_input);
