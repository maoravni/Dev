...
int CyBtldr_TransferData(unsigned char* inBuf, int inSize, unsigned char* outBuf, int outSize, int delay)
{
    ...
    if (CYRET_SUCCESS != err)
    {
        puts("CyBtldr_TransferData error");
...
int CyBtldr_ValidateRow(unsigned char arrayId, unsigned short rowNum)
{
    ...
    if (arrayId < MAX_FLASH_ARRAYS)
    {
        if (NO_FLASH_ARRAY_DATA == g_validRows[arrayId])
        {
            ...
            if (CYRET_SUCCESS == err)
                err = CyBtldr_TransferData(inBuf, inSize, outBuf, outSize, 0);
            if (CYRET_SUCCESS == err)
...
int CyBtldr_StartBootloadOperation(CyBtldr_CommunicationsData* comm, unsigned long expSiId, unsigned char expSiRev,
        unsigned long* blVer)
{
...
int CyBtldr_ProgramRow(unsigned char arrayID, unsigned short rowNum, unsigned char* buf, unsigned short size)
{
    ...
    if (CYRET_SUCCESS == err)
    {
        ...
        if (CYRET_SUCCESS == err)
            err = CyBtldr_TransferData(inBuf, inSize, outBuf, outSize, 30);
        if (CYRET_SUCCESS == err)
