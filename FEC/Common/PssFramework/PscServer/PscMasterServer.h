...
private:
    ...
    CTcpConnectorSocket *m_pTcpConnector; //!< TCP Connector for the server
...
//    void sendMonitorNotificationMessage(uint16_t pssId, E_DataTypes dataType, int16_t value);
    ...
    void sendErrorWithInfo(uint16_t boardId, uint16_t pssId, uint16_t secondaryPssId, uint32_t errors, uint32_t additionalErrors, char dataType, const void* dataValue);
    void sendWarning(uint16_t boardId, uint16_t pssId, uint16_t secondaryPssId, uint32_t warnings);
    void sendWarningWithInfo(uint16_t boardId, uint16_t pssId, uint16_t secondaryPssId, uint32_t warnings, uint32_t additionalWarnings, char dataType, const void* dataValue);
