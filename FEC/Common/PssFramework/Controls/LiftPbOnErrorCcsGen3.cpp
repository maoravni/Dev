...
#define M_LOWER_PRESSURE_LIMIT 0.2
    ...
    if (m_pbOk->getPssId() != 0)
        raiseErrorSimple(m_pbOk->getPssId(), E_PSSErrors_PrintBarLifted, !pbOk);
    else
...
void LiftPbOnErrorCcsGen3::serialize(F_FILE* f)
{
    Serializer<LiftPbOnErrorCcsGen3> s;
    s.serialize(f, *this);
}

LiftPbOnErrorCcsGen3::LiftPbOnErrorCcsGen3(F_FILE* f)
{
    Serializer<LiftPbOnErrorCcsGen3> s;
