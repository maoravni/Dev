/******************************************************************************
*                     (C) LandaLabs 2011                                      *
*******************************************************************************
*
*  PROJECT     : PHANTOM 
*
*  FILE        : usart.c 
*   
*  PATH        : C:\Landalabs\Spitfire\Embedded\Projects\Phantom\SW\BSP  
*
*  PURPOSE     : Usart extentions for StdPeriph_Driver. 
*
*  TOOLCHAIN:  : IAR Embedded Workbench for ARM. Version 6.30.1.3142
*
*  TARGET      : PHANTOM 
*
*
*  DATE        NAME              VER.    COMMENTS
* -----------------------------------------------------------------------------
*  2012.01     Igor Natachanny   1.0.0   Initial version
*
*******************************************************************************
      Copyright and Proprietary Statment. 
*******************************************************************************/
/*============================================================================*/
/*                  * L O C A L   C O N F I G U R A T I O N *                 */
/*============================================================================*/
/*============================================================================*/
/*                                * I M P O R T *                             */
/*============================================================================*/
/* --- GLOBAL_INCLUDE_FILES ------------------------------------------------- */
  #include <string.h>
  #include <stdint.h>
  #include <stdio.h>
  #include "stdarg.h"
  #include <assert.h>
#include "stm32f2xx.h"
  #include "ll_typedef.h"
  #include "free_rtos_exp.h"
  #include "mb_cnfg.h"
  #include "..\..\..\Common\MODBUS\Rtu\rtu_slave.h"
  #include "..\..\..\Common\MODBUS\Rtu\rtu_master.h"
  #include "..\..\..\Common\MODBUS\modbus_exp.h"
/* --- PACKAGE_INCLUDE_FILES ------------------------------------------------ */
  #include "usart.h"

/* --- GLOBAL_EXTERNALS ----------------------------------------------------- */
  extern xSemaphoreHandle xSmphrTx, xSmphrRx;
  

/* --- PACKAGE_EXTERNALS ---------------------------------------------------- */
/*============================================================================*/
/*                                * E X P O R T *                             */
/*============================================================================*/
/* --- PUBLIC_DATA ---------------------------------------------------------- */

/* --- PUBLIC_FUNCTIONS ----------------------------------------------------- */
/*============================================================================*/
/*                              * L O C A L S *                               */
/*============================================================================*/
  
/* --- PRIVATE_CONSTANTS ---------------------------------------------------- */
  
//  #define BUF_SIZE 10
 
/* --- PRIVATE MACROS ------------------------------------------------------- */
#define IS_MOSQUITO_MB_USART(USART) (((USART) == USART6) || \
                                     ((USART) == USART3)) 
/* --- PRIVATE TYPES -------------------------------------------------------- */
  
  struct buf_st {
    int in;                            // Next In Index
    int out;                           // Next Out Index
    char buf [MB_BUF_SZ];                       // Buffer
  };
  
/* --- PRIVATE FUNCTIONS ---------------------------------------------------- */
  
  void init_buf(struct buf_st *p);
  int32_t get_char(struct buf_st *p, uint8_t* c_ptr);
  int32_t put_char(struct buf_st *p, uint8_t c);
  
/* --- PRIVATE_DATA --------------------------------------------------------- */
  
  static struct buf_st   rbuf_1 = { 0, 0, };
  static struct buf_st   tbuf_1 = { 0, 0, };
  
  static struct buf_st   rbuf_3 = { 0, 0, };
  static struct buf_st   tbuf_3 = { 0, 0, };

  static struct buf_st   rbuf_6 = { 0, 0, };
  static struct buf_st   tbuf_6 = { 0, 0, };
  
 
  static uint8_t mb2_req_buf[MB_BUF_SZ];
  static uint8_t mb2_rsp_buf[MB_BUF_SZ];
 
/*============================================================================*/
/*     * P U B L I C   F U N C T I O N S   I M P L E M E N T A T I O N *      */
/*============================================================================*/ 
 /*----------------------------------------------------------------------------*
  Reads "size" bytes from Tx buffer of "USARTx" to specified by "data_ptr" memo-
  ry area. A user will responsible to this pointer validity. 
  Returns: -1 (fault) if this USART is not supported,
           -2 (fault) if "data_ptr" = 0,
           -3 (fault) if "size" = 0,   
            0 (success) - else.  
 *----------------------------------------------------------------------------*/
  
  uint32_t rcv_buf(uint8_t *data_ptr, uint32_t size, USART_TypeDef *USARTx){
   uint32_t res;
  
    while (size != 0){
      
      do{
        res = rcv_char(USARTx, data_ptr);
	    }while(res);
      
      size--;
      data_ptr++;
    }
   
   return res;
  }
  
/*----------------------------------------------------------------------------*
  Writes "size" bytes specified by "data_ptr"  to a Tx buffer of "USARTx".
  Returns: -1 (fault) if this USART is not supported,
           -2 (fault) if "data_ptr" = 0,
           -3 (fault) if "size" = 0,   
            0 (success) - else.  
 *----------------------------------------------------------------------------*/
  
  uint32_t send_buf(uint8_t *data_ptr, uint32_t size, USART_TypeDef *USARTx){
   uint32_t res;
  
    while (size != 0){
      
      do{
        res = send_char(USARTx, *data_ptr);
	    }while(res);
      
      size--;
      data_ptr++;
   }
   
    //taskYIELD();
   return res;
  }
  
/*----------------------------------------------------------------------------
  Clears the rx/tx buffers and all errors of the selected USART.
  Returns: -1 (fault) if this USART is not supported, 0 (success) - else.
 *----------------------------------------------------------------------------*/
  
  uint32_t init_usart( USART_TypeDef *USARTx ){
   struct buf_st *rx_buf_ptr = 0, *tx_buf_ptr = 0;
        
    if(USARTx == USART1){    
      rx_buf_ptr = &rbuf_1;
      tx_buf_ptr = &tbuf_1;
    }else{
      if(USARTx == USART3){
        rx_buf_ptr = &rbuf_3;
        tx_buf_ptr = &tbuf_3;
      }else{
        if(USARTx == USART6){
          rx_buf_ptr = &rbuf_6;
          tx_buf_ptr = &tbuf_6;
        }
          else 
              return(-1);/* USART is not supported */
         }
     }
    
    rx_buf_ptr->in = rx_buf_ptr->out = 0;
    tx_buf_ptr->in = tx_buf_ptr->out = 0;
    
    // Init variables
//    Usart6SerRxClr();
//    Usart6SerTxClr();
    
//    Usart6SerErrors = 0;
    
    return(0);/* success */

  }/* init_usart */

/*------------------------------------------------------------------------------
  Retrive first received character from apropriate receive buffer.
  Returns: -1 (fault) if a buffer is empty, -2(fault) if wrong port number, 0 (success) - else.
 *----------------------------------------------------------------------------*/
  int32_t rcv_char (USART_TypeDef *USARTx, uint8_t *c_ptr) {
   struct buf_st *p = 0;
   volatile int32_t res;

    if(USARTx == USART1)    
      p = &rbuf_1;
        else{
         if(USARTx == USART3)
	  p = &rbuf_3;
          else{
            if(USARTx == USART6)
              p = &rbuf_6;
            else 
              return(-2);/* wrong port number */
         }
     }

    res = get_char(p, c_ptr);

    return (res);
  }

/*------------------------------------------------------------------------------
  Puts a character to be sent into apropriate transmit buffer.
  Returns: -1 (fault) if a buffer is full, -2(fault) if wrong port number, 0 (success) - else.
 *------------------------------------------------------------------------------*/
  int32_t send_char(USART_TypeDef *USARTx, uint8_t c){
   struct buf_st *p = 0;
   volatile int32_t res;

    if(USARTx == USART1)    
      p = &tbuf_1;
        else{
         if(USARTx == USART3)
	         p = &tbuf_3;
          else{
            if(USARTx == USART6)
              p = &tbuf_6;
            else 
              return(-2);/* wrong port number */
         }
     }		          
	
    res = put_char(p, c);

    USARTx->CR1 |= USART_FLAG_TXE;


    return (res);
  }

/*============================================================================*/
/*     * P R I V A T E   F U N C T I O N S   I M P L E M E N T A T I O N *    */
/*============================================================================*/

/*-----------------------------------------------------------------------------
  This function handles USART6 global interrupt request.
  USART6 is used as AUX_TERM here.
 *----------------------------------------------------------------------------*/  
  void USART6_IRQHandler(){
    if (
        ((USART6->SR & USART_FLAG_TXE) != 0)
        &&
        ((USART6->CR1 & USART_FLAG_TXE) != 0)          
        ){
           if ( tbuf_6.out == tbuf_6.in )
  	         USART6->CR1 &= ~USART_FLAG_TXE;		      // disable TX interrupt if nothing to send
           else{
                 USART6->DR = tbuf_6.buf[tbuf_6.out];
                 tbuf_6.out++;
	               if( tbuf_6.out == MB_BUF_SZ )
	               tbuf_6.out = 0;  
           }
    }
    
    if ((USART6->SR & USART_FLAG_RXNE) != 0){   // Rx interrupt
      rbuf_6.buf[rbuf_6.in] = USART6->DR & 0x1FF;
	    rbuf_6.in++;
	    if( rbuf_6.in == MB_BUF_SZ )
	      rbuf_6.in = 0;
    }
    
    if ((USART6->SR & USART_FLAG_IDLE) != 0){   // Idle frame is detected

    }
    
  }/* void USART6_IRQHandler() */  

/*-----------------------------------------------------------------------------
  This function handles USART3 global interrupt request.
  USART3 is used as Modbus port#2 there. Modbus Slave port.
 *----------------------------------------------------------------------------*/ 
  void USART3_IRQHandler(){
   portBASE_TYPE xHigherPriorityTaskWoken= pdFALSE;
   static MB_SLAVE_IN_QUE_ITM_ST   cur_req;
   uint32_t status_reg_val = 0;
   int dummy = 0;
    
    status_reg_val = USART6->SR;/* as first step in a sequence for TC and IDLE flags reset */
    
    /*** Read data register not empty ***/
    if ((USART3->CR1 & USART_FLAG_RXNE) != 0){      
      if ((status_reg_val & USART_FLAG_RXNE) != 0){   // Rx interrupt
        USART_ITConfig(USART3, USART_IT_IDLE, ENABLE);
        rbuf_3.buf[rbuf_3.in] = USART3->DR & 0x1FF;
	      if( rbuf_3.in < MB_BUF_SZ)
	        rbuf_3.in ++;
        else
	        rbuf_3.out = 0;        
      }
    }/* if ((USART3->CR1 & USART_FLAG_RXNE) */
    
    /*** IDLE line detected ***/
    if ((USART3->CR1 & USART_FLAG_IDLE) != 0){
      if ((status_reg_val & USART_FLAG_IDLE) != 0){      
 /* Dummy Data Register reading for complete the sequence for IDLE bit Reset. */
        dummy = USART3->DR & 0x1FF; dummy++;/* this for compiler warning prevent */
        if(rbuf_3.buf[0] == slave_addr){
          cur_req.req_ptr = mb2_req_buf;
          cur_req.rsp_ptr = mb2_rsp_buf;
          cur_req.req_size = rbuf_3.in - 2;
          if(cur_req.req_size > 0){
            memcpy( cur_req.req_ptr, rbuf_3.buf, rbuf_3.in);
            xQueueSendFromISR( mb_node_2->queue, &cur_req, &xHigherPriorityTaskWoken );
                     
          }
          rbuf_3.out = rbuf_3.in = 0;

        }
      }
    }/* if ((USART3->CR1 & USART_FLAG_IDLE) */
   
    /*** Transmit data register empty ***/
    if ((USART3->CR1 & USART_FLAG_TXE) != 0){
      if((status_reg_val & USART_FLAG_TXE) != 0){
        if( tbuf_3.out != tbuf_3.in ){
         USART3->DR = tbuf_3.buf[tbuf_3.out];
	       tbuf_3.out++;
	       if( tbuf_3.out == MB_BUF_SZ )
            tbuf_3.out = 0;         
        }else{
//               tbuf_3.out = tbuf_3.in = 0;
               USART3->CR1 &= ~USART_FLAG_TXE;/* disable TX interrupt if nothing to send */
               USART_ITConfig(USART3, USART_IT_TC, ENABLE);
         }
      }
    }/* if ((USART3->CR1 & USART_FLAG_TXE) */
    
    /*** Transmission complete ***/
    if ((USART3->CR1 & USART_FLAG_TC) != 0){
      if((status_reg_val & USART_FLAG_TC) != 0){
/* Dummy Data Register write for complete the sequence for TC bit Reset. */
//        USART3->DR = 0;
        USART_ITConfig(USART3, USART_IT_TC, DISABLE);
        tbuf_3.out = tbuf_3.in = 0;        
        xSemaphoreGiveFromISR( mb_node_2->resp_tx_end_sem, &xHigherPriorityTaskWoken );
                    
      }
    }/* if ((USART3->CR1 & USART_FLAG_TC) */  
    
   /* Switch tasks if necessary. */	
    if( xHigherPriorityTaskWoken != pdFALSE )
    {
      portEND_SWITCHING_ISR( xHigherPriorityTaskWoken );
    }
  }/* void USART3_IRQHandler() */

/*-----------------------------------------------------------------------------
  This function handles USART1 global interrupt request. Modbus Master port.
  USART1 is used as Modbus port#1 there.
 *----------------------------------------------------------------------------*/
  void USART1_IRQHandler(){
   portBASE_TYPE xHigherPriorityTaskWoken=pdFALSE;
   uint32_t status_reg_val = 0;
   int dummy = 0;
    
    /*** Read data register not empty ***/
    if ((USART1->CR1 & USART_FLAG_RXNE) != 0){      
      if ((USART1->SR & USART_FLAG_RXNE) != 0){   // Rx interrupt
        USART_ITConfig(USART1, USART_IT_IDLE, ENABLE);
        rbuf_1.buf[rbuf_1.in] = USART1->DR & 0x1FF;
        mb_node_1->resp_len++;
	      if( rbuf_1.in < MB_BUF_SZ)
	        rbuf_1.in ++;
        else	        
	        rbuf_1.in = 0;        
      }
    }/* if ((USART1->CR1 & USART_FLAG_RXNE) */
    
    /*** IDLE line detected ***/
    if ((USART1->CR1 & USART_FLAG_IDLE) != 0){
      if ((USART1->SR & USART_FLAG_IDLE) != 0){
        status_reg_val = USART1->SR;/* as first step in a sequence for TC and IDLE flags reset */
 /* Dummy Data Register reading for complete the sequence for IDLE bit Reset. */
        dummy = USART1->DR & 0x1FF; dummy++;/* this for compiler warning prevent */
        status_reg_val += dummy;
        if(mb_node_1->resp_len > 3){
          memcpy( mb_node_1->resp_buf, rbuf_1.buf, rbuf_1.in);
//          mb_node_1->resp_len = rbuf_1.in;
          xSemaphoreGiveFromISR( mb_node_1->resp_rx_end_sem, &xHigherPriorityTaskWoken ); 
             
        }
        rbuf_1.out = rbuf_1.in = 0;        
      }
    }/* if ((USART1->CR1 & USART_FLAG_IDLE) */    

        /*** Transmit data register empty ***/
    if ((USART1->CR1 & USART_FLAG_TXE) != 0){
      if((USART1->SR & USART_FLAG_TXE) != 0){
        if( tbuf_1.out != tbuf_1.in ){
         USART1->DR = tbuf_1.buf[tbuf_1.out];
	       tbuf_1.out++;
	       if( tbuf_1.out == MB_BUF_SZ )
            tbuf_1.out = 0;         
        }else{
               tbuf_1.out = tbuf_1.in = 0;
               USART1->CR1 &= ~USART_FLAG_TXE;/* disable TX interrupt if nothing to send */
               USART_ITConfig(USART1, USART_IT_TC, ENABLE);
         }
      }
    }/* if ((USART1->CR1 & USART_FLAG_TXE) */
    
    /*** Transmission complete ***/
    if ((USART1->CR1 & USART_FLAG_TC) != 0){
      if((USART1->SR & USART_FLAG_TC) != 0){
/* Dummy Data Register write for complete the sequence for TC bit Reset. */
//        USART1->DR = 0;
        USART_ITConfig(USART1, USART_IT_TC, DISABLE);
        mb_set_dir(mb_node_1->name, RX);//xSemaphoreGiveFromISR( mb_node_1->req_tx_end_sem, &xHigherPriorityTaskWoken );
      }
    }/* if ((USART1->CR1 & USART_FLAG_TC) */
    
    /* Switch tasks if necessary. */	
    if( xHigherPriorityTaskWoken != pdFALSE )
    {
      portEND_SWITCHING_ISR( xHigherPriorityTaskWoken );
    }
  }/* void USART1_IRQHandler() */

/*------------------------------------------------------------------------------
  Clear com buffer indexes
 *------------------------------------------------------------------------------*/
  void init_buf(struct buf_st *p){

    p->in = 0; 
    p->out = 0;
  }

/*----------------------------------------------------------------------------
   Params:  a pointer to a desiried buffer,
   Returns: 0 (success);  -1 (fault) if a buffer is empty.
 *----------------------------------------------------------------------------*/
  int32_t get_char(struct buf_st *p, uint8_t* c_ptr){

    if ( p->out == p->in )
  	  return (-1);
	  else{
      *c_ptr = p->buf[p->out];
	    p->out++;
	    if( p->out == MB_BUF_SZ )
	      p->out = 0;
	     return (0);  
	  }
  }/* get_char */
/*----------------------------------------------------------------------------
  Params:  a pointer to a desiried buffer,  
  Returns: 0 (success);  -1 (fault) if a buffer is full. 
 *----------------------------------------------------------------------------*/
  int32_t put_char(struct buf_st *p, uint8_t c){
   unsigned int tmp_inx = p->in;

    tmp_inx++;
	if ( tmp_inx == MB_BUF_SZ )
	  tmp_inx = 0;

    if ( tmp_inx == p->out )
  	  return (-1);
    else{
      p->buf[p->in] = c;
	  p->in = tmp_inx;
	  return (0);  
	}
 }/* put_char */
/*============================================================================*/
/*                     (C) Copyright LandaLabs 2012                           */
/*=========================================================== end_of_file ====*/
   
