...
__attribute__ ((used,section(".flashops")))
int
stm32f4x_erase(struct flashinfo *fdev, int snum)
{
	while((FLASH->SR & FLASH_FLAG_BSY) == FLASH_FLAG_BSY);

    /* with the previous operation completed, proceed to erase the sector */
    FLASH->CR &= CR_PSIZE_MASK;
    FLASH->CR |= FLASH_PSIZE;
    FLASH->CR &= SECTOR_MASK;
    FLASH->CR |= (FLASH_CR_SER | (snum << 3));
    FLASH->CR |= FLASH_CR_STRT;
...
__attribute__ ((used,section(".flashops")))
int
stm32f4x_write(struct flashinfo *fdev,unsigned char *dest,unsigned char *src,long bytecnt)
{
	int i;
...
{
	uint16_t sval;
	uint32_t lval, addr = (uint32_t)dest;

	if (addr & 1) {
		if (FLASH_ProgramByte(addr++, *src++) != FLASH_COMPLETE)
			return(-1);
		bytecnt--;
	}
	if (bytecnt == 0)
	...
	if (addr & 3) {
		if (bytecnt > = 2) {
			memcpy((void*)&sval,src,2);
			if (FLASH_ProgramHalfWord(addr, val) != FLASH_COMPLETE)
				return(-1);
			addr += 2;
			src += 2;
			bytecnt -= 2;
	...
	/* at this point we've aligned the destination... */
	return(-1);
}
#endif
...
__attribute__ ((used,section(".flashops")))
int
stm32f4x_ewrite(struct flashinfo *fdev,unsigned char *dest,unsigned char *src,long bytecnt)
{
...
int
stm32f4x_type(struct flashinfo *fdev)
{
...
int
stm32f4x_lock(struct flashinfo *fdev,int snum,int operation)
{
...
struct flashdesc FlashNamId[] = {
	{ DUMMY_FLASHID,	"STM32F4X-Flash"}
};

struct sectorinfo sinfoSTM32F4X[SECTORTOT];
...
FlashInit(void)
{
	...
	fbnk->fltype = stm32f4x_type;
	fbnk->flerase = stm32f4x_erase;
	fbnk->flwrite = stm32f4x_write;
	fbnk->flewrite = stm32f4x_ewrite;
	fbnk->fllock = stm32f4x_lock;

	fbnk->sectors = sinfoSTM32F4X;
