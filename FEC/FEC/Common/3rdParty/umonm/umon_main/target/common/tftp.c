...
static char	TftpState;			/* Used to keep track of the current state
								 ...
								 */
static uchar *TftpD2TFS;		/* If non-zero, then the incoming transfer is
								 * written directly to TFS flash space.
								 */
...
processTFTP(struct ether_header *ehdr,ushort size)
{
...
#if INCLUDE_TFS
			if ((TftpAddr = tfsnxtdataptr()) == 0) {
				SendTFTPErr(ehdr,99,1,"TFS full");
				return(0);
			}
			TftpD2TFS = TftpAddr;
...
#else
			SendTFTPErr(ehdr,99,1,"TFS disabled");
			return(0);
#endif
		}
	...
	case ecs(TFTP_DAT):
		...
		if (InFlashSpace(TftpAddr,count)) {
            if (AppFlashWrite(TftpAddr,data,count) < 0) {
				SendTFTPErr(ehdr,0,1,"TFTP error writing to flash");
				TftpCount = -1;
				return(0);
			}
            TftpAddr += count;
		...
		if (count < TFTP_DATAMAX) {
			if (TftpTfsFname[0]) {
				...
				if (TftpD2TFS == 0) {
					...
					err = tfsadd(TftpTfsFname,info,flags,
						TftpD2TFS,TftpCount+1-TftpChopCount);
				}
				if (err != TFS_OKAY) {
...
#if INCLUDE_ETHERVERBOSE
		if (EtherVerbose & SHOW_TFTP_STATE) {
			ushort errcode = ecs(*(ushort *)(tftpp+2));
...
SendTFTPRRQ(uchar *ipadd,uchar *eadd,char *filename,char *mode,uchar *loc)
{
	...
	te->ether_type = ecs(ETHERTYPE_IPV4);
 ...
 *	Command line:
 *		tftp [options] {IP} {get} {file|addr} [len]...
...
char *TftpHelp[] = {
	...
	"-[aF:f:i:] [on|off|IP] {get filename [addr]}",
...
Tftp(int argc,char *argv[])
{
	...
	flags = (char *)0;
	mode = "octet";
	while ((opt=getopt(argc,argv,"aF:f:i:")) != -1) {
		switch(opt) {
		case 'a':
			mode = "netascii";
			break;
	...
	if (!strcmp(argv[optind+1],"get")) {
		...
		if (argc == optind+4)
			...
			addr = (ulong)tfsnxtdataptr();
			TftpD2TFS = (uchar *)addr;
		}
#endif
		else
			return(CMD_PARAM_ERROR);
	...
	else	/* This version does not support 'put' */
		return(CMD_PARAM_ERROR);
