...
extern uint8_t IPv6MulticastOnly;
...
/* ip protocols (applicable to ipv6 also) */
#define IP_IP		0
#define IP_ICMP 	1
#define IP_IGMP 	2
#define IP_TCP  	6
...
struct ipv6hdr {
	...
	uint8_t nxthdr;
	uint8_t hoplmt;
	uint8_t saddr[IPV6ADDRSZ];
	uint8_t daddr[IPV6ADDRSZ];
} __PACKED__;

#define IPV6_VERSION(a)			((a & 0xf0000000) >> 28)
#define IPV6_TRAFFIC_CLASS(a)	((a & 0x0ff00000) >> 20)
#define IPV6_FLOW_LABEL(a)		(a & 0x000fffff)
#define IPV6_PAYLOAD_LEN(a)		((a & 0xffff0000) >> 16)
#define IPV6_NEXT_HDR(a)		((a & 0x0000ff00) >> 8)
#define IPV6_HOP_LIMIT(a)		(a & 0x000000ff)

/* The ICMPV6 header begins with these three fields
 * (type/code/cksum); with more type-specific information
 * following.
 */
struct icmpv6hdr {
	uint8_t type;
...
struct Udp6Pseudohdr {
	uint8_t saddr[IPV6ADDRSZ];	/* source addr */
	uint8_t daddr[IPV6ADDRSZ];	/* dest addr */
	uint32_t ulplen;			/* upper layer packet len */
	uint32_t nxthdr;			/* next header field */
} __PACKET__ ;
...
extern	void udp6Chksum(struct ipv6hdr *);
...
extern	int printIpv6(struct ipv6hdr *);
...
extern	int processIPv6(struct ether_header *,uint16_t);
extern	int processICMP(struct ether_header *,unsigned short);
...
extern	void MacToLinkLocalIPV6Addr(uint8_t *ipv6addr, uint8_t *mac);
