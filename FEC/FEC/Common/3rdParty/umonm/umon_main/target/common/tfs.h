...
#ifndef TFSINFOSIZE
#define TFSINFOSIZE		11		/* Max size of info string (mod4-1). */
#endif
 ...
 *  This header is different than the normal 'uMon' header because of
 *  restrictions put on flash writes with the LPC1769.  It can only write
 *  to a 16-byte block ONCE because internally it does some ECC on the
 *  block so even though typical flash allows ANY bit to be cleared, 
 *  that rule is broken with the LPC1769 (and I'm guessing others in 
 *  this family as well).
 *  So, the header must be a size that is mod16, and also the altflags
 *  field must be in a 16-byte-aligned block all to itself (so that a
 *  file can be delted).
 */
struct tfshdr {
...
/* Macros: */
#define TFS_DELETED(fp)     (!((fp)->altflags[0] & TFS_ACTIVE))
#define TFS_FILEEXISTS(fp)  ((fp)->altflags[0] & TFS_ACTIVE)
#define TFS_ISEXEC(fp)		((fp)->flags & TFS_EXEC)
#define TFS_ISBOOT(fp)		((fp)->flags & TFS_BRUN)
#define TFS_STALE(fp) 		(!((fp)->altflags[0] & TFS_NSTALE))
