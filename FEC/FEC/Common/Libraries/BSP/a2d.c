...
/* --- PRIVATE_DATA --------------------------------------------------------- */
  ...
  uint32_t a2d_accum_arr[A2D_NUM];
  uint16_t a2d_smpls_arr[A2D_NUM];
/*============================================================================*/
/*                  * L O C A L   C O N F I G U R A T I O N *                 */
/*============================================================================*/
...
**/
  A2D_RES_ET  a2d_read_dig(A2D_DATA_ET param_id, uint16_t *val_ptr)
  {
    ...
    *val_ptr = a2d_results.whole[param_id];
    return A2D_COMPLETE;    
#if 0
      res = xSemaphoreTake(conv_end_sem, A2D_CONV_FINISH_SEM_TICK_TO_WAIT);
  ...
  uint32_t a2d_start( unsigned portSHORT stack_depth, unsigned portBASE_TYPE priority )
  {
   signed portBASE_TYPE res;
    ...
    for(uint32_t i = 0; i <A2D_NUM; i++) 
        a2d_accum_arr[i] = 0x00000000; /* Accumulator Array cleaning */
...
**/
 void a2d_init_hw(void)
 {
  ...
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_DMA2 | RCC_AHB1Periph_GPIOF | RCC_AHB1Periph_GPIOA | RCC_AHB1Periph_GPIOB | RCC_AHB1Periph_GPIOC, ENABLE);
  ...
  DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)&(a2d_smpls_arr[ADC3_in4]);
  ...
  DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)&(a2d_smpls_arr[ADC1_in0]);
  DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;
  ...
  //60Mhz/pres
  if(ADC_SMPLS_NUM < 128)div_par=ADC_Prescaler_Div8;
  else div_par=ADC_Prescaler_Div4;
  /* ADC Common Init **********************************************************/
   ...
   signed portBASE_TYPE res_1, res_2;
    ...
    while (1)
    {
      ...
      if(res_1 && res_2)
      {
        ...
        for(uint32_t i = 0; i < A2D_NUM; i++)
        {
          a2d_results.whole[i]=(a2d_accum_arr[i]>>ADC_AVG_DIV_FACTOR);
          a2d_accum_arr[i] = 0x00000000; /* Accumulator Array cleaning */
        }
        xSemaphoreGive(conv_end_sem);
      }
...
*****************************************************************************/
  void DMA2_Stream0_IRQHandler(void)//ADC1
  {
    ...
    if(++curr_sample_a2d_1 < ADC_SMPLS_NUM)
    {
      for(uint32_t i = FIRST_IN_CONV_1; i < FIRST_IN_CONV_2; i++) 
        a2d_accum_arr[i] += (a2d_smpls_arr[i] & 0x0FFF); /* Get least 12 bits */
    ...
    { 
       xSemaphoreGiveFromISR( a2d_sem_1, &xHigherPriorityTaskWoken );
    }
...
*******************************************************************************/
  void DMA2_Stream1_IRQHandler(void)//ADC3
  {
    ...
    if(++curr_sample_a2d_2 < ADC_SMPLS_NUM)
    {   
    ...
    { 
       xSemaphoreGiveFromISR( a2d_sem_2, &xHigherPriorityTaskWoken );
    }
