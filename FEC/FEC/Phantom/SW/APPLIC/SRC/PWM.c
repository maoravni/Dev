...
void PWM_init(int TaskPriority, int StackSize)
{
	...
	for (int Element=0; Element<MAX_NUM_OF_PWM_ELEMENTS;Element++)
		Pwm_CreateElement(Element, 0,0, PWM_DEFAULT_CYCLE);   //1 second cycle 
...
STATUS Pwm_SetDutyCycle(int ElementId, U8 PercentDutyCycle)
{
	...
	if (PwmData[ElementId].Enable)
		PwmData[ElementId].ResetPulseValue=PwmData[ElementId].SetPulseValue+(PercentDutyCycle*PwmData[ElementId].TotalPeriodValue)/100;
...
void Pwm_task(void * pvParameters){
	...
	while(1){      
			...
			// if ((ElementNum==0) && (PwmElementPtr->CurrentCounter==10))
		{
			counter+=1;
			counter_10ms=0;
			//sprintf( (char*)msg_buf,"RUN TEST %d seconds",counter);
			//aux_term_put_msg(msg_buf);
			M_LOGGER_LOGF_UNSORTED(0,"RUN TEST %d seconds",counter);
		...
		for (int ElementNum=0;ElementNum<MAX_NUM_OF_PWM_ELEMENTS;ElementNum++)
		{
			...
			if ((PwmElementPtr->ResetPulseValue==0) || ((PwmElementPtr->ResetPulseValue<=90) && (PwmElementPtr->CurrentCounter==PwmElementPtr->ResetPulseValue)))
                                dig_out_ctrl(ElementNum, DEACTIVE); //turn it off
				//(PwmElementPtr->OutputPortId)->BSRRL = PwmElementPtr->OutputPinId;  
			else
				if ((PwmElementPtr->ResetPulseValue>90) || ((PwmElementPtr->ResetPulseValue!=0) && (PwmElementPtr->CurrentCounter==PwmElementPtr->SetPulseValue)))
