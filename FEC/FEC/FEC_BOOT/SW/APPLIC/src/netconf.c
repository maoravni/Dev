...
extern  int  IP_BYTE_H,IP_BYTE_M1,IP_BYTE_M2,IP_BYTE_L;
...
void LwIP_Init(void)
{
  ...
  gw.addr = 0;
#else
    IP4_ADDR(&ipaddr, IP_BYTE_H, IP_BYTE_M1, IP_BYTE_M2, IP_BYTE_L);
//  IP4_ADDR(&ipaddr, IP_ADDR0, IP_ADDR1, IP_ADDR2, IP_ADDR3);
  IP4_ADDR(&netmask, NETMASK_ADDR0, NETMASK_ADDR1 , NETMASK_ADDR2, NETMASK_ADDR3);
  IP4_ADDR(&gw, GW_ADDR0, GW_ADDR1, GW_ADDR2, GW_ADDR3);

 #ifdef USE_LCD  
   iptab[0] = IP_BYTE_L ;//IP_ADDR3;
   iptab[1] = IP_BYTE_M2;//IP_ADDR2;
   iptab[2] = IP_BYTE_M1;//IP_ADDR1;
   iptab[3] = IP_BYTE_H ;//IP_ADDR0;
...
void LwIP_Periodic_Handle(__IO uint32_t localtime)
{
  ...
  /* Toggle DIP_LED every 500 ms */
  if ((localtime - LED_Timer) >= LED_TMR_PERIOD)
  {
    LED_Timer =  localtime;
