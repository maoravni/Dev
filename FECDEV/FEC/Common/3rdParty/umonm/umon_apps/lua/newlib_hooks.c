...
_open_r(REENT *rptr, const char *path, int flags, int mode)
{
	...
	if (strlen(pathcopy) > TFSNAMESIZE) {
		rptr->_errno = ENAMETOOLONG;
		mon_printf("_open_r err 2\n");
		return(-1);
	}
	if (istr) {
		if (strlen(istr) > TFSNAMESIZE) {
			rptr->_errno = ENAMETOOLONG;
			mon_printf("_open_r err 3\n");
			return(-1);
		}
	}
...
_write_r(REENT *rptr, int fd, const void *ptr, size_t len)
{
	...
	if ((fd <= 0) || (fd >= MAXTFDS)) {
		rptr->_errno = EBADF;
		mon_printf("tfswrite fd=%d\n",fd);
		return(-1);
	}

	ret = mon_tfswrite(tfdtable[fd].tfd,(char *)ptr,len);
	if (ret < 0) {
		mon_printf("tfswrite failed\n");
		return(-1);
	}
	else
		return(len);
...
_fstat_r(REENT *rptr, int fd, struct stat *sp)
{
	...
	if ((fd == 1) || (fd == 0))
		return(0);
	if ((fd < MAXTFDS) && tfdtable[fd].inuse)
		return(0);
...
int
putenv(char *string)	/* takes a "name=value" string */
{
