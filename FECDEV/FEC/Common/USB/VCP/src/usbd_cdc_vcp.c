...
extern xQueueHandle         que_hndl_rx;
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
...
static uint16_t VCP_DataTx (uint8_t* Buf, uint32_t Len)
{
 if( USB_OTG_dev.dev.connection_status ==0 )return USBD_FAIL;//Igor 29/10/2012
 
 if(APP_Rx_ptr_out == APP_Rx_ptr_in) 
 {
    APP_Rx_ptr_out = APP_Rx_ptr_in = 0;
 }

 if ((APP_RX_DATA_SIZE - APP_Rx_ptr_in) < Len)
	  return USBD_BUSY;
...
static uint16_t VCP_DataRx (uint8_t* Buf, uint32_t Len)
{
 ...
 static  uint32_t j=0;
   ...
   for (i=0; i < Len; i++){
     ...
     j=0;
     flars |=USB_MCG_READY;
     xQueueSend( que_hndl_rx, msg_buff, 0 );//dummy     
     break;
    }
  } 
